...
var init = function(dbConn) {

    var tasksSchema = dbConn.Schema({
        userID: String,
        task: String,
        done: Boolean
    });
    dbConnection = dbConn.model('Tasks', tasksSchema);
...
dMVC.TaskModel = dMVC.Model.subClass({
    ...
    deleteByID: function(id) {
        dbConnection.remove({_id: id}, function (err) {
            if (err) {
                //res.json({error: err});
            } else {
    ...
    getUserTasks: function(userID, callback) {
        dbConnection.find({userID: userID},function (err, tasks) {
            if (err) {
                callback({error: err});
            } else {
                callback(tasks);
...
dMVC.TaskController = dMVC.Controller.subClass({
    ...
    deleteTask: function(req, res, next) {
        this.modelInstance.deleteByID(req.body.modelID);
        res.json([{
            command: 'delete',
            id: req.body.modelID
        }]);
